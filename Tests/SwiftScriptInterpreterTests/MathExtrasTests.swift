...
struct MathExtrasTests {
    ...
    @Test func gcd() async throws {
        let interp = Interpreter()
        #expect(try await interp.eval("gcd(48, 18)") == .int(6))
        #expect(try await interp.eval("gcd(0, 5)") == .int(5))
        #expect(try await interp.eval("gcd(-12, 8)") == .int(4))
    }

    @Test func lcm() async throws {
        let interp = Interpreter()
        #expect(try await interp.eval("lcm(4, 6)") == .int(12))
        #expect(try await interp.eval("lcm(0, 5)") == .int(0))
    }

    @Test func factorial() async throws {
        let interp = Interpreter()
        #expect(try await interp.eval("factorial(0)") == .int(1))
        #expect(try await interp.eval("factorial(6)") == .int(720))
    }

    @Test func binomial() async throws {
        let interp = Interpreter()
        #expect(try await interp.eval("binomial(10, 3)") == .int(120))
        #expect(try await interp.eval("binomial(5, 0)") == .int(1))
        #expect(try await interp.eval("binomial(5, 5)") == .int(1))
    }
    ...
    @Test func doubleClamped() async throws {
        let interp = Interpreter()
        let r = try await interp.eval("3.5.clamped(to: 0...3)")
        #expect(r == .double(3.0))
    }

    @Test func doubleSign() async throws {
        let interp = Interpreter()
        let r = try await interp.eval("((-3.5).sign, 0.0.sign, 3.5.sign)")
        #expect(r == .tuple([.double(-1.0), .double(0.0), .double(1.0)]))
    }

    @Test func truncatingRemainder() async throws {
        let interp = Interpreter()
        let r = try await interp.eval("10.5.truncatingRemainder(dividingBy: 3.0)")
    ...
    @Test func arraySumInt() async throws {
        let interp = Interpreter()
        #expect(try await interp.eval("[1, 2, 3, 4].sum()") == .int(10))
    }

    @Test func arraySumDouble() async throws {
        let interp = Interpreter()
        #expect(try await interp.eval("[1.5, 2.5, 3.0].sum()") == .double(7.0))
    }

    @Test func arrayProduct() async throws {
        let interp = Interpreter()
        #expect(try await interp.eval("[1, 2, 3, 4].product()") == .int(24))
    }

    @Test func arrayAverage() async throws {
        let interp = Interpreter()
        #expect(try await interp.eval("[2.0, 4.0, 6.0].average()") == .double(4.0))
    }
...
struct StatisticsTests {
    @Test func median() async throws {
        let interp = Interpreter()
        #expect(try await interp.eval("[3.0, 1.0, 4.0, 1.0, 5.0, 9.0, 2.0, 6.0].median()") == .double(3.5))
        #expect(try await interp.eval("[1.0, 2.0, 3.0].median()") == .double(2.0))
    }

    @Test func variance() async throws {
        let interp = Interpreter()
        #expect(try await interp.eval("[2.0, 4.0, 4.0, 4.0, 5.0, 5.0, 7.0, 9.0].variance()") == .double(4.0))
    }

    @Test func stdDev() async throws {
        let interp = Interpreter()
        #expect(try await interp.eval("[2.0, 4.0, 4.0, 4.0, 5.0, 5.0, 7.0, 9.0].stdDev()") == .double(2.0))
    }

    @Test func percentile() async throws {
        let interp = Interpreter()
        #expect(try await interp.eval("[1.0, 2.0, 3.0, 4.0].percentile(0.5)") == .double(2.5))
        #expect(try await interp.eval("[1.0, 2.0, 3.0, 4.0].percentile(0.0)") == .double(1.0))
        #expect(try await interp.eval("[1.0, 2.0, 3.0, 4.0].percentile(1.0)") == .double(4.0))
    }
    ...
    @Test func mixedIntArrayWorks() async throws {
        ...
        let interp = Interpreter()
        let r = try await interp.eval("[1, 2, 3, 4, 5].median()")
        #expect(r == .double(3.0))
    }
}
